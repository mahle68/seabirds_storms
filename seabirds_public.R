...
m_max <- lm(wing.loading..Nm.2. ~ range_max, data = lm_input) #adR = -0.04219  , p-value= 0.6367

m_median <- lm(wing.loading..Nm.2. ~ range_median, data = lm_input) #adR = 0.3463 , p-value= 0.003758

winds <- cor.test(lm_input$range_max, lm_input$range_median) #cor 0.079 , p = 0.7392


#difference between wind median and max
lm_input %>% 
  mutate(wind_diff = range_max/range_median) %>% 
  summarize(max_diff = max(wind_diff),
            min_diff = min(wind_diff))
