...
region_w1 <- world %>% 
  st_crop(xmin = 30, xmax = 70, ymin = -52, ymax = -27)  
...
plot_w1 <- ggplot() +
  ...
  theme(axis.text = element_text(size = 12, colour = 1),
        ...
        legend.position = c(0.08,0.23),
        legend.background = element_rect(colour = 1, fill = "white"))+
  labs(x = NULL, y = NULL, title = paste(wanderer1$sci_name, paste(wanderer1$timestamp, "UTC", sep = " "), sep = "   "))
...
region_s1 <- as(world, "Spatial")
region_s1 <- crop(region_s1, extent(-37, 9,-59, -35))

region_s1 <- world %>% 
  st_crop(ymin = -55, ymax = -30, xmin = -37, xmax = 9)  
...
#create main map
plot_ayn <- ggplot() +
  geom_raster(data = ayn, aes(x = lon, y = lat, fill = wind_speed))+
  geom_segment(data = ayn, 
               aes(x = lon, xend = lon+u10/10, y = lat, 
                   yend = lat+v10/10), arrow = arrow(length = unit(0.12, "cm")), size = 0.3)+
  geom_sf(data = world, fill = "grey85")+
  ...
  theme(axis.text = element_text(size = 12, colour = 1),
        ...
        legend.background = element_rect(colour = 1, fill = "white"),
        plot.title = element_text(face = "italic"))+
  labs(x = NULL, y = NULL, title = paste(ayn_pt$sci_name, paste(ayn_pt$timestamp, "UTC", sep = " "), sep = "   "))

#create inset map
worldmap_ayn <- ggplot() + 
  geom_polygon(data = world.df, aes(x = long, y = lat, group = group), fill = "grey80") +
  geom_polygon(data = region_ayn_df, aes(x = x, y = y), fill = NA, color = "black", size = 0.3) + 
  coord_map("ortho", orientation = c(ayn_pt$location.lat+37, ayn_pt$location.long, 0)) +
  scale_y_continuous(breaks = (-2:2) * 30) +
  scale_x_continuous(breaks = (-4:4) * 45) +
  theme_minimal() +
  theme(axis.text = element_blank(),
...
final_ayn <- ggdraw() +
  draw_plot(plot_ayn) +
  draw_plot(worldmap_ayn, x = 0.69, y = 0.70, width = 0.25, height = 0.25)
