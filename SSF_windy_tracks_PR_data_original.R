...
data_an <- PR_tot %>% 
  filter(common_name %in% c("Atlantic Petrel", "Soft-plumaged Petrel"))


# STEP 2: prepare alternative steps#####

move_ls<-lapply(split(data_an, data_an$common_name),function(x){
...
mycl <- makeCluster(detectCores() - 10) #7 cores, one for each species
...
save(used_av_ls, file = paste0("/home/enourani/ownCloud/Work/Projects/seabirds_and_storms/R_files/PR_2spp_alt_steps_", hrs, "hr_", n, "n",".RData"))
...
colnames(used_av_all)[c(5,4)] <- c("location-long","location-lat") #rename columns to match movebank format

write.csv(used_av_all, paste0("/home/enourani/ownCloud/Work/Projects/seabirds_and_storms/R_files/PR_2spp_alt_steps_", hrs, "hr_", n, "n",".csv"))
...
ann_cmpl <- read.csv("/home/enourani/ownCloud/Work/Projects/seabirds_and_storms/annotation/PR_2spp_2hr/PR_2spp_alt_steps_2hr_20n.csv-6315167699594627873.csv", stringsAsFactors = F) %>% 
...
save(ann_cmpl, file = paste0("/home/enourani/ownCloud/Work/Projects/seabirds_and_storms/R_files/PR_2spp_data_ssf_ann_", hrs, "hr_", n, "n",".RData"))
...
models <- lapply(split(ann_cmpl, ann_cmpl$species), function(x){
  ...
  save(m, file = paste0("/home/enourani/ownCloud/Work/Projects/seabirds_and_storms/R_files/", x$species[1], "_", hrs, "_hr", "_m4_full.RData")) #n of 50 for binned wind
  ...
  jpeg(paste0("/home/enourani/ownCloud/Work/Projects/seabirds_and_storms/INLA_results_figures/", x$species[1], "_",hrs ,"hrs_all_smooth_0_1_abs_cw.jpeg"), 
       width = 11, height = 3.5, units = "in", res = 300)
  ...
  plot(x = x$abs_cross_wind_kmh, y = x$used, pch = 16,  col = adjustcolor("grey", alpha.f = 0.1), xlab = "abs(cross wind) (kmh)", ylab = "exp(y)")
  lines(cw$x,exp(cw$y)) 
