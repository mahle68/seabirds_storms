...
save(used_av_all_1hr, file = "/home/enourani/ownCloud/Work/Projects/seabirds_and_storms/R_files/used_av_2hr_PR.RData")
...
colnames(used_av_all_1hr)[c(4,3)] <- c("location-long","location-lat") #rename columns to match movebank format

write.csv(used_av_all_1hr, "/home/enourani/ownCloud/Work/Projects/seabirds_and_storms/R_files/ssf_PR_all_spp_2hr.csv")
...
ann_cmpl <- read.csv("/home/enourani/ownCloud/Work/Projects/seabirds_and_storms/annotation/Peter Ryan/ssf/ssf_PR_all_spp_2hr.csv-7110251487308386619/ssf_PR_all_spp_2hr.csv-7110251487308386619.csv", stringsAsFactors = F) %>% 
  rename(sst = ECMWF.Interim.Full.Daily.SFC.Sea.Surface.Temperature,
         t2m = ECMWF.Interim.Full.Daily.SFC.Temperature..2.m.above.Ground.,
...
#for plotting
ann_cmpl$common_name <- factor(ann_cmpl$species)
levels(ann_cmpl$common_name) <- c("Great shearwater", "Tristan albatross", "Sooty albatross", "Grey petrel", "Atlantic petrel", "Soft-plumaged petrel", "Atlantic yellow-nosed albatross")


#plot
jpeg("/home/enourani/ownCloud/Work/Projects/seabirds_and_storms/SSF_process_figures/Used_avail_PR_2hr.jpeg", width = 12, height = 10, units = "in", res = 300)
     
X11(width = 12, height = 10)
par(mfrow= c(3,1), oma = c(0,0,3,0))
for(i in c("wind_support_kmh", "cross_wind_kmh","wind_speed_kmh")){
  
  boxplot(ann_cmpl[,i] ~ ann_cmpl[,"common_name"], data = ann_cmpl, boxfill = NA, border = NA, main = i, xlab = "", ylab = "")
  #if(i == "wind_support_kmh"){
    legend("bottomleft", legend = c("used","available"), fill = c("orange","gray"), bty = "n")
  #}
  boxplot(ann_cmpl[ann_cmpl$used == 1,i] ~ ann_cmpl[ann_cmpl$used == 1,"common_name"], 
          xaxt = "n", add = T, boxfill = "orange",
          boxwex = 0.25, at = 1:length(unique(ann_cmpl$common_name)) - 0.15)
  boxplot(ann_cmpl[ann_cmpl$used == 0,i] ~ ann_cmpl[ann_cmpl$used == 0,"common_name"], 
          xaxt = "n", add = T, boxfill = "grey",
          boxwex = 0.25, at = 1:length(unique(ann_cmpl$common_name)) + 0.15)
  #  } 
}
mtext("Used and available wind conditions (2 hr)", side = 3, outer = T, cex = 1.3)
dev.off()
